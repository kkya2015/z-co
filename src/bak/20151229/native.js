...
(function($L, global) {
	 ...
	 * 启动一个app
	 */
	$L.openApp = function(params, callback) {
		$L.executeNativeJS(['app', 'openApp'], params, callback);
	}

	/*
	 * 判断设备上是否已安装指定app
	 */
	$L.isAppInstalled = function(params, callback) {
		$L.executeNativeJS(['app', 'isAppInstalled'], params, function(isInstalled) {
        ...
        addAveragePowerCallback: function(callback) {
          $L.executeObjFunJS([recorder, 'addAveragePowerCallback'], function(averagePower) {
            if ($L.isFunction(callback)) {
              callback.call(null, averagePower);
            }
          });
        },
        /*
